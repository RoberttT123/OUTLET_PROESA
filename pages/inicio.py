# -*- coding: utf-8 -*-
"""
PANEL DE CONTROL E INVENTARIO - OUTLET PROESA
----------------------------------------------
Página de administración: métricas, visualización del catálogo
y carga/sincronización del inventario maestro.

NOTA: set_page_config e inject_nav_css están en app.py (entry point).
...
# ══════════════════════════════════════════════════════════════════════════════
# PROCESADOR SEGURO PARA TEXTO SIN FORMATO
# Evita conversiones automáticas de fecha en celdas de precio/stock.
# ══════════════════════════════════════════════════════════════════════════════
def sanitizar_matriz_inventario(df, s_col_idx=3, p_col_idx=4):
    ...
    for idx, row in df.iterrows():
        ...
        # ── Precio ────────────────────────────────────────────────────────────
        try:
            p_str = str(val_precio).upper().replace("BS", "").strip()
            # Formato latino: 1.500,00  →  separador miles=punto, decimal=coma
            if re.match(r'^\d{1,3}(\.\d{3})+(,\d+)?$', p_str):
                p_str = p_str.replace(".", "").replace(",", ".")
            elif "," in p_str and "." not in p_str:
                p_str = p_str.replace(",", ".")
            else:
                p_str = p_str.replace(",", "")
...
# ══════════════════════════════════════════════════════════════════════════════
# CARGA DESDE LA NUBE — con caché de tiempo real
# ══════════════════════════════════════════════════════════════════════════════
@st.cache_data(ttl=CACHE_TTL_SEGUNDOS, show_spinner=False)
def obtener_inventario_sheets_protegido(url, hoja):
    """Descarga datos de Google Sheets congelando el resultado temporalmente."""
...
def escribir_inventario_sheets(url_sheet: str, hoja: str, df_nuevo: pd.DataFrame) -> bool:
    try:
        from src.sheets import _con_reintento
...
# ══════════════════════════════════════════════════════════════════════════════
# CARGA CON DOBLE ESCUDO: caché en session_state + caché @st.cache_data
# ══════════════════════════════════════════════════════════════════════════════
def cargar_inventario_visto():
    """
    Carga el inventario en session_state si está vacío o si venció el TTL.
    Prioridad: nube → Excel local.
    """
    if "df_inventario_maestro" in st.session_state and st.session_state.df_inventario_maestro is not None:
        ...
        if ts and (datetime.now() - ts).total_seconds() < CACHE_TTL_SEGUNDOS:
            return   # Datos vigentes, no hace falta refrescar
    ...
    if USING_SHEETS:
        try:
            ...
            pass  # Fallback al Excel local si la nube falla
...
# Ejecutar carga al inicio de la página
...
# ══════════════════════════════════════════════════════════════════════════════
# SECCIÓN: CARGA DE CATÁLOGO MAESTRO
# ══════════════════════════════════════════════════════════════════════════════
...
archivo = st.file_uploader(
    "Sube el Excel de inventario del mes (Hoja1) para reescribir la base local y Google Sheets:",
    type=["xlsx"],
    ...
    if USING_SHEETS:
        with status_container.container():
            with st.spinner("🚀 Sincronizando nuevo catálogo con Google Sheets en la nube..."):
                exito_nube = escribir_inventario_sheets(
                    INVENTARIO_SHEET_URL, INVENTARIO_HOJA_NAME, df_sanitizado
                )
                if exito_nube:
                    st.session_state.mostrar_toast_exito = True
                else:
                    st.warning("⚠️ Guardado localmente, pero falló la escritura en Google Sheets.")

    # Invalida solo el caché del inventario, sin afectar otras funciones cacheadas
    obtener_inventario_sheets_protegido.clear()
...
# ── Guardia: sin inventario, nada que mostrar ─────────────────────────────────
if df_inv is None:
...
valor_total = (
    pd.to_numeric(df_inv[stock_col],  errors="coerce").fillna(0)
    * pd.to_numeric(df_inv[precio_col], errors="coerce").fillna(0.0)
).sum()
...
with col4:
    ...
    <div class="metric-card amber">
        ...
        <div class="metric-sub">en {n_empresas} empresa{'s' if n_empresas != 1 else ''}</div>
    </div>""", unsafe_allow_html=True)
...
<div class="info-banner">
    ...
    (Sincronizado con la nube: {ts_str}).
    Las filas en <span style="background:#FEE2E2;padding:1px 5px;border-radius:3px">rojo</span>
    indican stock agotado,
    en <span style="background:#FEF9C3;padding:1px 5px;border-radius:3px">amarillo</span>
    stock bajo (≤5 unidades).
    👉 Ve a <strong>Mi Pedido</strong> en el menú lateral para ingresar pedidos.
</div>
""", unsafe_allow_html=True)